...
public partial class BackReconciledExceptions : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (IsPostBack == false)
            {
                ...
                //LoadData();
                bll.LoadVendorsToReconcile(cboBank);
    ...
    private void Comment()
    {
        ...
        {
            datafile.ExecuteNonQuery("LogBackReconComment", id, comment, user);
            MultiView1.ActiveViewIndex = 0;
            SearchDb();
        }
    }
    ...
    private void LoadRpt()
    {
        ...
        try
        {
            ...
            {
                bll.ExportToExcel(dataTable, "", Response);
            }
            else if (rdPdf.Checked)
            {
                bll.ExportToPdf(dataTable, "", Response);
            }
        }
        catch (Exception ex)
        {
