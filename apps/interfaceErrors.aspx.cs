...
namespace apps
{
    public partial class DistrictTotals : System.Web.UI.Page
    {
        ...
        {
            dataTable = datafile.getInterfaceErrorUtilities();
            DataRow myDataRow = dataTable.NewRow();
            myDataRow["Utility"] = "ALL UTILITIES";
            dataTable.Rows.InsertAt(myDataRow, 0);
            cboStatus.DataSource = dataTable;
            cboStatus.DataValueField = "Utility";
            cboStatus.DataTextField = "Utility";
            cboStatus.DataBind();
            if (dataTable.Rows.Count > 0)
            {
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    string district = dataTable.Rows[0]["Utility"].ToString();
                    cboStatus.SelectedIndex = cboStatus.Items.IndexOf(cboStatus.Items.FindByValue(district));
                }
                else
                {
        ...
        {
            dataTable = datafile.getInterfaceErrorVendorCodes();
            DataRow myDataRow = dataTable.NewRow();
            myDataRow["VendorCode"] = "ALL VENDOR CODES";
            dataTable.Rows.InsertAt(myDataRow, 0);
            cboBranches.DataSource = dataTable;
            cboBranches.DataValueField = "VendorCode";
            cboBranches.DataTextField = "VendorCode";
            ...
            if (dataTable.Rows.Count > 0)
            {
                if (dataTable.Rows.Count == 1)
                {
                    ...
                    string district = dataTable.Rows[0]["VendorCode"].ToString();
