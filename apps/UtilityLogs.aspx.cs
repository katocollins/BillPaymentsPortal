...
public partial class UtilityLogs : System.Web.UI.Page
{
    ...
    private void PopulateGridView(DataTable datatable, object[] data)
    {
        if (datatable.Rows.Count > 0)
        {
            ...
            UtilityLogsGridView.DataSource = datatable;
            UtilityLogsGridView.DataBind();
            UtilityLogsGridView.UseAccessibleHeader = true;
            UtilityLogsGridView.HeaderRow.TableSection = TableRowSection.TableHeader;
            ClearErrorMessage();
        }
        else
        {
    ...
    private void LoadAllUtilityLogs()
    {
    ...
    protected string SearchResultMessage(string id, string idName, string sdate, string edate)
    {
        ...
        message = String.Format("Currently, there are no Utility logs {0}", resp);
        return message;
