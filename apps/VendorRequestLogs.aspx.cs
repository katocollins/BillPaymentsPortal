...
namespace apps
{
    public partial class VendorRequestLogs : System.Web.UI.Page
    {
        string username = "";
        string fullname = "";
        string vendor = "";
        ...
        private void PopulateGridView(DataTable datatable, object[] data)
        {
            if (datatable.Rows.Count > 0)
            {
                ...
                VendorRequestsGridView.DataSource = datatable;
                VendorRequestsGridView.DataBind();
                VendorRequestsGridView.UseAccessibleHeader = true;
                VendorRequestsGridView.HeaderRow.TableSection = TableRowSection.TableHeader;
                ClearErrorMessage();
            }
            else
            {
        ...
        private void LoadAllVendorRequestsLogs()
        {
            string tranId, start_date, end_date;
            try
            {
                tranId = "";
                start_date = "";
                end_date = "";
                object[] data = { tranId, start_date, end_date };
        ...
        private object[] GetSearchParameters()
        {
            ...
            string start_date = startDate.Text.Trim();
            string end_date = endDate.Text.Trim();
        ...
        protected string SearchResultMessage(string id, string idName, string sdate, string edate)
        {
            ...
            message = String.Format("Currently, there are no vendor request logs {0}", resp);
            return message;
