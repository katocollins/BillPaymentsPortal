...
namespace apps
{
    public partial class MakeManualPayment : System.Web.UI.Page
    {
        ...
        private void ToggleToken()
        {
            if (bll.IsvalidToken())
            {
                ...
                txtDistrict.Text = Session["DistrictName"].ToString();
                txtCustRef.Focus();
                LoadPayTypes();
            }
            else
        ...
        private void PostTransaction()
        {
            ...
            string mreceipto = txtReceiptno.Text.Trim();
            bool cancelled = chkcancelled.Checked;
            if (custref.Equals(""))
            {
            ...
            else if (mreceipto.Equals(""))
            {
                ShowMessage("Please Enter Manual Receipt Number", true);
                txtReceiptno.Focus();
            }
            //else if (bll.ManualReceiptExists(mreceipto))
            //{
            //    ShowMessage("Duplicate Manual Receipt Number", true);
            //    txtReceiptno.Focus();
            //}
            ...
            else
            {
                ...
                t.Offline = "1";
                string receipt_status = bll.isValidManualReceipt(mreceipto, t.TranAmount);
                if (receipt_status.Equals("YES"))
                {
                    res = bll.PostPayment(t, bal.ToString(), mreceipto, cancelled, true);
                    if (res.Errorcode.Equals("0"))
                    {
                        ...
                        ClearContrls();
                        ShowMessage("Payment Posted Successfully. Receipt Number " + receiptno, false);
                ...
                {
                    ShowMessage(receipt_status, true);
                }
            }
        }
