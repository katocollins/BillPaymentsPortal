...
namespace apps
{
    public partial class UssdLogs : System.Web.UI.Page
    {
        string username = "";
        string fullname = "";
        string vendor = "";
        ...
        private void PopulateGridView(DataTable datatable, object[] data)
        {
            if (datatable.Rows.Count > 0)
            {
                ...
                UssdLogsGridView.DataSource = datatable;
                UssdLogsGridView.DataBind();
                UssdLogsGridView.UseAccessibleHeader = true;
                UssdLogsGridView.HeaderRow.TableSection = TableRowSection.TableHeader;
                ClearErrorMessage();
            }
            else
            {
        ...
        private void LoadAllUssdLogs()
        {
            string VendorTranId, vendorCode, phoneNumber, paymentRef, status;
            string fromNetwork, toNetwork, telecomId, start_date, end_date;
            ...
            try
            {
                ...
                object[] data = { VendorTranId, vendorCode, phoneNumber, paymentRef, status,
                              fromNetwork, toNetwork, telecomId, start_date, end_date };
        ...
        protected DataTable SearchDB(object[] data)
        {
            ...
            try
            {
                DataSet ds = bll.ExecuteDataAccess("LivePegasusUssd", "GetUssdLogs", data);
                dt = ds.Tables[0];
            }
            catch (Exception ex)
            {
        ...
        private object[] GetSearchParameters()
        {
            ...
            VendorTranId = TranId.Text.ToString();
            vendorCode = VendorCode.Text.ToString();
            phoneNumber = PhoneNumber.Text.ToString();
            paymentRef = PaymentReference.Text.ToString();

            status = Status.Text.ToString();
            fromNetwork = FromNetwork.Text.ToString();
            toNetwork = ToNetwork.Text.ToString();
            telecomId = TelecomId.Text.ToString();
            ...
            searchCriteria.Add(paymentRef);
            searchCriteria.Add(status);
            searchCriteria.Add(fromNetwork);
            searchCriteria.Add(toNetwork);
        ...
        protected string SearchResultMessage(string id, string idName, string sdate, string edate)
        {
            ...
            message = String.Format("Currently, there are no USSD logs {0}", resp);
            return message;
